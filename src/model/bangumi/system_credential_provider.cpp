...
#include <utility>
    ...
    #include "model/bangumi/secret_service_prompt_p.hpp"
...
auto systemCredentialError(QString message) -> BangumiError {
    return bangumiError(BangumiErrorCode::CredentialStoreError, std::move(message));
}
 ...
 */
auto operator<<(QDBusArgument &argument, const SecretServiceSecret &secret) -> QDBusArgument & {
 ...
 */
auto operator>>(const QDBusArgument &argument, SecretServiceSecret &secret) -> const QDBusArgument & {
    argument.beginStructure();
    argument >> secret.session >> secret.parameters >> secret.value >> secret.contentType;
    argument.endStructure();
    return argument;
...
public:
     ...
     */
    SecretServiceCredentialProvider() : mConnection(QDBusConnection::sessionBus()) {
    ...
    auto load() -> BangumiResult<std::optional<QByteArray>> override {
        ...
        if (found->unlocked.isEmpty()) {
            ...
            return ilias::Err(systemCredentialError(QStringLiteral("系统凭据条目仍处于锁定状态")));
        }

        auto reply = call(found->unlocked.front().path(), QString::fromLatin1(kItemInterface),
                          QStringLiteral("GetSecret"), {QVariant::fromValue(*session)});
        if (!reply) {
            return ilias::Err(std::move(reply.error()));
        }
        if (reply->arguments().size() != 1) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 GetSecret 响应格式无效")));
        }

        const auto secret = qdbus_cast<SecretServiceSecret>(reply->arguments().front());
        if (secret.value.isEmpty() ||
            secret.value.size() > kMaximumCredentialSize) {
            auto message = QStringLiteral("系统凭据大小无效：%1 bytes").arg(secret.value.size());
            return ilias::Err(systemCredentialError(message));
        }
    ...
    auto save(const QByteArray &data) -> BangumiResult<void> override {
        if (data.isEmpty() || data.size() > kMaximumCredentialSize) {
            auto message = QStringLiteral("拒绝保存大小无效的系统凭据：%1 bytes").arg(data.size());
            return ilias::Err(systemCredentialError(message));
        }
        ...
        properties.insert(QStringLiteral("org.freedesktop.Secret.Item.Label"), QStringLiteral("anime-land Bangumi OAuth token"));
        properties.insert(QStringLiteral("org.freedesktop.Secret.Item.Attributes"), QVariant::fromValue(attributes()));
        ...
        auto reply = call(collection->path(), QString::fromLatin1(kCollectionInterface), QStringLiteral("CreateItem"),
                          {properties, QVariant::fromValue(secret), true});
        if (!reply) {
            return ilias::Err(std::move(reply.error()));
        }
        if (reply->arguments().size() != 2) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 CreateItem 响应格式无效")));
        }
    ...
    auto clear() -> BangumiResult<void> override {
        ...
        if (!found->locked.isEmpty()) {
            return ilias::Err(systemCredentialError(QStringLiteral("无法解锁要删除的系统凭据条目")));
        }

        for (const auto &item : found->unlocked) {
            auto reply = call(item.path(), QString::fromLatin1(kItemInterface), QStringLiteral("Delete"), {});
            if (!reply) {
                return ilias::Err(std::move(reply.error()));
            }
            if (reply->arguments().size() != 1) {
                return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 Delete 响应格式无效")));
            }
            auto prompted = completePrompt(qdbus_cast<QDBusObjectPath>(reply->arguments().front()));
            if (!prompted) {
                return ilias::Err(std::move(prompted.error()));
            }
        }
...
private:
     ...
     */
    auto call(const QString &path, const QString &interface, const QString &method,
              const QList<QVariant> &arguments) -> BangumiResult<QDBusMessage> {
        if (!mConnection.isConnected()) {
            auto message = QStringLiteral("无法连接当前用户的 D-Bus session：%1").arg(mConnection.lastError().message());
            return ilias::Err(systemCredentialError(message));
        }
        auto message = QDBusMessage::createMethodCall(QString::fromLatin1(kServiceName), path, interface, method);
        message.setArguments(arguments);
        auto reply = mConnection.call(message, QDBus::Block, kMethodTimeoutMilliseconds);
        if (reply.type() == QDBusMessage::ErrorMessage) {
            auto error = QStringLiteral("Secret Service %1 失败（%2）").arg(method, reply.errorName());
            return ilias::Err(systemCredentialError(error));
        }
        return reply;
    }
    ...
    auto openSession() -> BangumiResult<QDBusObjectPath> {
        auto reply = call(QString::fromLatin1(kServicePath), QString::fromLatin1(kServiceInterface),
                          QStringLiteral("OpenSession"), {QStringLiteral("plain"), QVariant::fromValue(QDBusVariant(QString {}))});
        if (!reply) {
            return ilias::Err(std::move(reply.error()));
        }
        if (reply->arguments().size() != 2) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 OpenSession 响应格式无效")));
        }
        const auto path = qdbus_cast<QDBusObjectPath>(reply->arguments().at(1));
        if (path.path().isEmpty() || path.path() == QStringLiteral("/")) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 未返回有效 session")));
        }
        return path;
    }
    ...
    void closeSession(const QDBusObjectPath &path) noexcept {
        auto message = QDBusMessage::createMethodCall(QString::fromLatin1(kServiceName), path.path(),
    ...
    auto searchItems() -> BangumiResult<SearchResult> {
        auto reply = call(QString::fromLatin1(kServicePath), QString::fromLatin1(kServiceInterface),
                          QStringLiteral("SearchItems"), {QVariant::fromValue(attributes())});
        if (!reply) {
            return ilias::Err(std::move(reply.error()));
        }
        if (reply->arguments().size() != 2) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 SearchItems 响应格式无效")));
        }
        return SearchResult {
    ...
    auto unlock(const ObjectPaths &paths) -> BangumiResult<void> {
        ...
        if (reply->arguments().size() != 2) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 Unlock 响应格式无效")));
        }
        auto prompted = completePrompt(qdbus_cast<QDBusObjectPath>(reply->arguments().at(1)));
        if (!prompted) {
            return ilias::Err(std::move(prompted.error()));
        }
        return {};
    ...
    auto defaultCollection() -> BangumiResult<QDBusObjectPath> {
        ...
        if (reply->arguments().size() != 1) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 ReadAlias 响应格式无效")));
        }
        auto collection = qdbus_cast<QDBusObjectPath>(reply->arguments().front());
        if (!collection.path().isEmpty() && collection.path() != QStringLiteral("/")) {
            return collection;
        }

        QVariantMap properties;
        properties.insert(QStringLiteral("org.freedesktop.Secret.Collection.Label"), QStringLiteral("Default keyring"));
        reply = call(QString::fromLatin1(kServicePath), QString::fromLatin1(kServiceInterface), QStringLiteral("CreateCollection"),
                     {properties, QStringLiteral("default")});
        if (!reply) {
            return ilias::Err(std::move(reply.error()));
        }
        if (reply->arguments().size() != 2) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 的 CreateCollection 响应格式无效")));
        }
        collection = qdbus_cast<QDBusObjectPath>(reply->arguments().at(0));
        auto promptResult = completePrompt(qdbus_cast<QDBusObjectPath>(reply->arguments().at(1)));
        if (!promptResult) {
        ...
        if (collection.path().isEmpty() || collection.path() == QStringLiteral("/")) {
            return ilias::Err(systemCredentialError(QStringLiteral("Secret Service 未返回有效的默认 collection")));
        }
        return collection;
    }
     ...
     */
    auto completePrompt(const QDBusObjectPath &prompt) -> BangumiResult<QVariant> {
        ...
        if (!mConnection.connect(QString::fromLatin1(kServiceName), prompt.path(), QString::fromLatin1(kPromptInterface),
                                 QStringLiteral("Completed"), &waiter, SLOT(completed(bool, QDBusVariant)))) {
            return ilias::Err(systemCredentialError(QStringLiteral("无法监听 Secret Service 系统提示")));
        }
        auto disconnect = qScopeGuard([this, &prompt, &waiter]() {
            mConnection.disconnect(QString::fromLatin1(kServiceName), prompt.path(), QString::fromLatin1(kPromptInterface),
                                   QStringLiteral("Completed"), &waiter, SLOT(completed(bool, QDBusVariant)));
        });

        auto reply = call(prompt.path(), QString::fromLatin1(kPromptInterface), QStringLiteral("Prompt"), {QString {}});
        if (!reply) {
            return ilias::Err(std::move(reply.error()));
        }
        ...
        if (!waiter.isCompleted()) {
            return ilias::Err(systemCredentialError(QStringLiteral("等待系统凭据提示超时")));
        }
        if (waiter.isDismissed()) {
            return ilias::Err(systemCredentialError(QStringLiteral("系统凭据提示已取消")));
        }
...
auto windowsErrorMessage(DWORD code) -> QString {
    ...
    const auto size = FormatMessageW(flags, nullptr, code, 0, reinterpret_cast<wchar_t *>(&buffer), 0, nullptr);
    QString message;
    if (size == 0) {
...
public:
    ...
    auto load() -> BangumiResult<std::optional<QByteArray>> override {
        ...
        if (!CredReadW(kCredentialTarget, CRED_TYPE_GENERIC, 0, &credential)) {
            ...
            auto message = QStringLiteral("读取 Windows Credential Manager 失败：%1").arg(windowsErrorMessage(error));
            return ilias::Err(systemCredentialError(message));
        }
        const std::unique_ptr<CREDENTIALW, decltype(&CredFree)> holder(credential, &CredFree);
        if (credential->CredentialBlobSize == 0 ||
            credential->CredentialBlobSize >
                static_cast<DWORD>(kMaximumCredentialSize)) {
            auto message = QStringLiteral("Windows 系统凭据大小无效：%1 bytes").arg(credential->CredentialBlobSize);
            return ilias::Err(systemCredentialError(message));
        }
        QByteArray data(reinterpret_cast<const char *>(credential->CredentialBlob), static_cast<qsizetype>(credential->CredentialBlobSize));
        return std::optional<QByteArray> {std::move(data)};
    }
    ...
    auto save(const QByteArray &data) -> BangumiResult<void> override {
        if (data.isEmpty() || data.size() > kMaximumCredentialSize ||
            ...
            auto message = QStringLiteral("拒绝保存大小无效的 Windows 系统凭据：%1 bytes").arg(data.size());
            return ilias::Err(systemCredentialError(message));
        }
        ...
        if (!CredWriteW(&credential, 0)) {
            const auto error = GetLastError();
            auto message = QStringLiteral("保存 Windows Credential Manager 失败：%1").arg(windowsErrorMessage(error));
            return ilias::Err(systemCredentialError(message));
        }
        return {};
    }
    ...
    auto clear() -> BangumiResult<void> override {
        ...
        auto message = QStringLiteral("删除 Windows Credential Manager 凭据失败：%1").arg(windowsErrorMessage(error));
        return ilias::Err(systemCredentialError(message));
    }
};
...
auto securityErrorMessage(OSStatus status) -> QString {
    ...
    CFStringGetCharacters(message, CFRangeMake(0, length), reinterpret_cast<UniChar *>(result.data()));
    CFRelease(message);
    return result;
}
...
public:
    ...
    auto load() -> BangumiResult<std::optional<QByteArray>> override {
        ...
        if (status != errSecSuccess) {
            auto message = QStringLiteral("读取 macOS Keychain 失败：%1").arg(securityErrorMessage(status));
            return ilias::Err(systemCredentialError(message));
        }
        ...
        if (size <= 0 || size > kMaximumCredentialSize) {
            CFRelease(value);
            auto message = QStringLiteral("macOS 系统凭据大小无效：%1 bytes").arg(size);
            return ilias::Err(systemCredentialError(message));
        }
        QByteArray result(reinterpret_cast<const char *>(CFDataGetBytePtr(data)), static_cast<qsizetype>(size));
        CFRelease(value);
    ...
    auto save(const QByteArray &data) -> BangumiResult<void> override {
        if (data.isEmpty() || data.size() > kMaximumCredentialSize) {
            auto message = QStringLiteral("拒绝保存大小无效的 macOS 系统凭据：%1 bytes").arg(data.size());
            return ilias::Err(systemCredentialError(message));
        }
        const auto value = CFDataCreate(kCFAllocatorDefault, reinterpret_cast<const UInt8 *>(data.constData()),
        ...
        if (status == errSecItemNotFound) {
            ...
            CFDictionarySetValue(query, kSecAttrLabel, CFSTR("anime-land Bangumi OAuth token"));
        ...
        CFRelease(value);
        if (status != errSecSuccess) {
            auto message = QStringLiteral("保存 macOS Keychain 失败：%1").arg(securityErrorMessage(status));
            return ilias::Err(systemCredentialError(message));
        }
        return {};
    }
    ...
    auto clear() -> BangumiResult<void> override {
        ...
        auto message = QStringLiteral("删除 macOS Keychain 凭据失败：%1").arg(securityErrorMessage(status));
        return ilias::Err(systemCredentialError(message));
    }
...
private:
    ...
    static auto baseQuery() -> CFMutableDictionaryRef {
        ...
        CFDictionarySetValue(query, kSecAttrAccount, CFSTR("model/bangumi/oauth-token"));
        return query;
    }
};
...
auto createPlatformSystemCredentialProvider() -> BangumiResult<std::unique_ptr<SystemCredentialProvider>> {
#if defined(Q_OS_LINUX)
...
#else
    return ilias::Err(bangumiError(BangumiErrorCode::UnsupportedCredentialStore, QStringLiteral("当前平台构建不支持系统凭据库")));
#endif
}
