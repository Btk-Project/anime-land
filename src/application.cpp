...
auto commandName(const cli::Command &command) -> std::string_view {
    return std::visit(
        [](const auto &value) -> std::string_view {
            ...
            else if constexpr (std::is_same_v<Command, cli::GuiCommand>) {
                return "gui";
            }
            else {
                return "search";
...
auto applyLogSettings(const GeneralSettings &settings,
                      const std::optional<std::string> &levelOverride = {})
    -> bool {
    const std::string_view level = levelOverride ? *levelOverride
                                                 : settings.log_level;
    const auto configured = configureLogging(
        ...
        {.directory = expandVariables(settings.log_file_path),
         ...
         .maxFileCount = settings.log_file_max_count > 0
                             ? static_cast<std::size_t>(
                                   settings.log_file_max_count)
                             : 1U});
    if (!configured.success) {
        AL_LOG_WARN("[app.log] file logging unavailable: {}",
                    configured.errorMessage);
        return false;
    }
    AL_LOG_INFO("[app.log] file logging ready path={}",
                configured.filePath.string());
    return true;
}
...
auto runGraphicalApplication(
    QGuiApplication &application,
    const std::optional<std::string> &logLevelOverride) -> int {
    ...
    static_cast<void>(applyLogSettings(generalSettings, logLevelOverride));
    ...
    const auto networkCacheOptions =
        bangumiNetworkCacheOptions(bangumiSettings);
...
auto anime_land::runApplication(int argc, char **argv) -> int {

    auto parsed = argc == 1
                      ? ParseOutcome {
                            .command = std::optional {cli::GuiCommand {}},
                            .exitCode = 0,
                        }
    ...
    if (needsBrowser) {
        ...
        const int exitCode =
            runGraphicalApplication(application, runtimeOptions.logLevel);
    ...
    static_cast<void>(applyLogSettings(generalSettings,
                                       runtimeOptions.logLevel));
