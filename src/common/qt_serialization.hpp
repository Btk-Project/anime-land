...
inline auto assignUtf8(std::string_view utf8, QString &value) -> ParserResult {
  if (utf8.size() >
    ...
    return detail::parser_error(sa::ErrorCode::InvalidLength,
                                "UTF-8 string is too large for QString");
  }
  QStringDecoder decoder(QStringDecoder::Utf8);
  QString parsed =
      decoder(QByteArrayView(utf8.data(), static_cast<qsizetype>(utf8.size())));
  if (decoder.hasError()) {
    return detail::parser_error(sa::ErrorCode::ParseError,
 ...
 * 不修改目标 QString。
 */
template <> struct CustomParser<QString> {
  /**
   * @brief 写入 QString。
 ...
 */
template <> struct CustomParser<QUrl> {
  /**
  ...
  static auto write(W &writer, const QUrl &value, const Parent &parent,
    ...
    if (!value.isValid()) {
      return detail::parser_error(sa::ErrorCode::InvalidType,
                                  "Cannot serialize an invalid QUrl");
    }
    return parser_write<W>(writer, value.toString(QUrl::FullyEncoded), parent,
                           tags);
  }
  ...
  static auto read(typename R::InputValueType input, QUrl &value,
    ...
    if (!parsed.isValid()) {
      ...
      return detail::parser_error(
          sa::ErrorCode::ParseError,
          "Invalid QUrl: " +
              std::string(reason.constData(),
                          static_cast<std::size_t>(reason.size())));
    }
    value = std::move(parsed);
    return sa::success();
  }
