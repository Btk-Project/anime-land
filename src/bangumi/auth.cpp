...
auto qString(std::string_view value) -> QString {
...
auto constantTimeEqual(const QByteArray &left, const QByteArray &right)
    -> bool {
  return left.size() == right.size() &&
         sodium_memcmp(left.constData(), right.constData(),
                       static_cast<std::size_t>(left.size())) == 0;
}
...
auto makeState() -> QString {
  ...
  return QString::fromLatin1(
      QByteArray(reinterpret_cast<const char *>(random.data()),
                 static_cast<qsizetype>(sizeof(random)))
          .toHex());
}
  ...
  if (!base.isValid() || base.scheme() != QStringLiteral("https") ||
    ...
    return ilias::Err(
        bangumiError(BangumiErrorCode::InvalidConfiguration,
                     QStringLiteral("无效的 Bangumi HTTPS Base URL：%1")
                         .arg(base.toString())));
  }
...
auto networkError(QNetworkReply &reply, QStringView operation) -> BangumiError {
  if (reply.error() == QNetworkReply::OperationCanceledError) {
    return bangumiError(BangumiErrorCode::Cancelled,
                        QStringLiteral("%1已取消").arg(operation));
  }
  return bangumiError(
      BangumiErrorCode::NetworkError,
      QStringLiteral("%1失败：%2").arg(operation, reply.errorString()));
}

auto applicationConfigurationMessage(const BangumiSettings &settings,
                                     QStringView reason) -> QString {
  return QStringLiteral("%1。请在 %2 确认应用仍存在、App ID/App Secret 正确，"
                        "且回调地址严格等于 %3")
      .arg(reason, settings.oauth_application_page.toString(),
 ...
 */
auto locallyValidateOAuthApplication(const BangumiSettings &settings)
    -> BangumiResult<void> {
  const QString &clientId = settings.client_id;
  const QString clientSecret = qString(settings.client_secret);
  ...
  if (clientId.isEmpty() || clientSecret.isEmpty()) {
    return ilias::Err(bangumiError(
        ...
        applicationConfigurationMessage(
            settings, QStringLiteral("Bangumi App ID/App Secret 尚未配置"))));
  }
  if (clientId.size() > 512 || clientSecret.size() > 4096 ||
    ...
    return ilias::Err(bangumiError(
        ...
        applicationConfigurationMessage(
            settings,
            QStringLiteral(
                "Bangumi App ID/App Secret 含空白、控制字符或长度异常"))));
  }
  ...
  if (!applicationPage.isValid() ||
    ...
    return ilias::Err(bangumiError(
        BangumiErrorCode::InvalidConfiguration,
        QStringLiteral("oauth_application_page 必须是有效的 HTTPS URL")));
  }
  return {};
}
 ...
 */
auto tokenExchangeConfigurationError(const QByteArray &data, int status,
                                     const BangumiSettings &settings)
    -> std::optional<BangumiError> {
  ...
  if (status == 400 || status == 401 ||
    ...
    return bangumiError(
        ...
        applicationConfigurationMessage(
            settings,
            QStringLiteral("Bangumi 拒绝了 OAuth Token 交换（HTTP %1）")
                .arg(status)));
  }
  return std::nullopt;
}
 ...
 */
auto buildBangumiAuthorizationUrl(const BangumiSettings &settings,
                                  const QString &state) -> BangumiResult<QUrl> {
  ...
  if (state.isEmpty()) {
    return ilias::Err(bangumiError(BangumiErrorCode::InvalidConfiguration,
                                   QStringLiteral("OAuth state 不能为空")));
  }
  ...
  if (!redirect.isValid() || redirect.scheme() != QStringLiteral("http") ||
    ...
    return ilias::Err(bangumiError(
        BangumiErrorCode::InvalidConfiguration,
        QStringLiteral("redirect_uri 必须是带固定端口和路径的 HTTP 回环地址")));
  }
...
auto inspectBangumiAuthorizationPage(const QByteArray &response,
                                     const BangumiSettings &settings)
    -> BangumiResult<void> {
  ...
  if (lower.contains("app_nonexistence") ||
    ...
    return ilias::Err(bangumiError(
        ...
        applicationConfigurationMessage(
            settings,
            QStringLiteral(
                "Bangumi 返回 app_nonexistence，App ID 不存在或应用已删除"))));
  }
  return {};
}

auto parseBangumiCallbackRequest(const QByteArray &request,
                                 const QString &expectedPath,
                                 const QString &expectedState)
    -> BangumiResult<QString> {
  if (request.size() > kMaximumCallbackRequestSize) {
    return ilias::Err(bangumiError(BangumiErrorCode::InvalidCallback,
                                   QStringLiteral("OAuth 回调请求过大")));
  }

  const qsizetype lineEnd = request.indexOf("\r\n");
  if (lineEnd <= 0) {
    return ilias::Err(bangumiError(BangumiErrorCode::InvalidCallback,
                                   QStringLiteral("OAuth 回调请求行不完整")));
  }
  const auto parts = request.first(lineEnd).split(' ');
  if (parts.size() != 3 || parts[0] != "GET" ||
      !parts[2].startsWith("HTTP/1.")) {
    return ilias::Err(
        bangumiError(BangumiErrorCode::InvalidCallback,
                     QStringLiteral("OAuth 回调只接受 HTTP GET")));
  }

  const QUrl target = QUrl::fromEncoded(parts[1], QUrl::StrictMode);
  if (!target.isValid() || target.path() != expectedPath) {
    return ilias::Err(bangumiError(BangumiErrorCode::InvalidCallback,
                                   QStringLiteral("OAuth 回调路径不匹配")));
  }
  ...
  if (!constantTimeEqual(returnedState.toUtf8(), expectedState.toUtf8())) {
    ...
    return ilias::Err(bangumiError(BangumiErrorCode::InvalidCallback,
                                   QStringLiteral("OAuth state 校验失败")));
  }
  ...
  if (query.hasQueryItem(QStringLiteral("error"))) {
    ...
    return ilias::Err(bangumiError(
        BangumiErrorCode::AuthorizationDenied,
        QStringLiteral("Bangumi 授权未完成：%1").arg(description)));
  }

  const QString code = query.queryItemValue(QStringLiteral("code"));
  if (code.isEmpty()) {
    return ilias::Err(bangumiError(BangumiErrorCode::InvalidCallback,
                                   QStringLiteral("OAuth 回调缺少 code")));
  }
  return code;
}
...
auto BangumiAuth::login(std::chrono::nanoseconds callback_timeout)
  ...
  AL_LOG_DEBUG(
      "[bangumi.oauth] authorization flow requested timeout_seconds={}",
      std::chrono::duration_cast<std::chrono::seconds>(callback_timeout)
          .count());
  if (mActive) {
    AL_LOG_WARN("[bangumi.oauth] authorization flow already active");
    co_return ilias::Err(
        bangumiError(BangumiErrorCode::InvalidState,
                     QStringLiteral("Bangumi 登录已在进行中")));
  }
  mActive = true;
  LoginGuard guard{*this};

  mExpectedState = makeState();
  auto authorizationUrl =
      detail::buildBangumiAuthorizationUrl(mSettings, mExpectedState);
  if (!authorizationUrl) {
    AL_LOG_ERROR("[bangumi.oauth] local configuration validation failed "
                 "code={}",
                 bangumiErrorCodeName(authorizationUrl.error().code));
  ...
  if (!isLoopbackHost(redirect.host(), listenAddress)) {
    ...
    co_return ilias::Err(
        bangumiError(BangumiErrorCode::InvalidConfiguration,
                     QStringLiteral("redirect_uri 不是回环地址")));
  }
  mExpectedCallbackPath = redirect.path();
  if (!mCallbackServer.listen(listenAddress,
                              static_cast<quint16>(redirect.port()))) {
    AL_LOG_ERROR(
        "[bangumi.oauth] callback listener failed port={} socket_error={}",
        redirect.port(), static_cast<int>(mCallbackServer.serverError()));
    co_return ilias::Err(bangumiError(
        BangumiErrorCode::CallbackListenFailed,
        QStringLiteral("无法监听 OAuth 回调 %1：%2")
            .arg(redirect.authority(), mCallbackServer.errorString())));
  }
  AL_LOG_INFO("[bangumi.oauth] callback listener ready port={}",
              redirect.port());
  ...
  if (!QDesktopServices::openUrl(*authorizationUrl)) {
    ...
    co_return ilias::Err(
        bangumiError(BangumiErrorCode::BrowserOpenFailed,
                     QStringLiteral("无法打开系统默认浏览器")));
  }
  ...
  if (cancelled.has_value()) {
    ...
    co_return ilias::Err(bangumiError(BangumiErrorCode::Cancelled,
                                      QStringLiteral("Bangumi 登录已取消")));
  }
  if (timeout.has_value()) {
    AL_LOG_WARN("[bangumi.oauth] browser callback timed out");
    co_return ilias::Err(bangumiError(BangumiErrorCode::CallbackTimeout,
                                      QStringLiteral("等待浏览器回调超时")));
  }
  if (!callback.has_value()) {
    AL_LOG_WARN("[bangumi.oauth] callback wait stopped without a result");
    co_return ilias::Err(bangumiError(BangumiErrorCode::Cancelled,
                                      QStringLiteral("浏览器回调等待已停止")));
  }
  auto callbackResult = std::move(*callback);
  if (!callbackResult) {
    AL_LOG_WARN("[bangumi.oauth] callback rejected code={}",
                bangumiErrorCodeName(callbackResult.error().code));
...
auto BangumiAuth::waitForCallback() -> ilias::Task<BangumiResult<QString>> {
  if (!mCallbackServer.hasPendingConnections()) {
    const auto signal = co_await ilias_qt::QSignal(&mCallbackServer,
                                                   &QTcpServer::newConnection);
    if (!signal) {
      co_return ilias::Err(
          bangumiError(BangumiErrorCode::Cancelled,
                       QStringLiteral("OAuth 回调服务器已销毁")));
    }
  }
  ...
  if (socket == nullptr) {
    ...
    co_return ilias::Err(bangumiError(BangumiErrorCode::InvalidCallback,
                                      QStringLiteral("OAuth 回调连接不可用")));
  }
  struct SocketGuard {
    QTcpSocket *socket;
  ...
  if (!request) {
    AL_LOG_WARN("[bangumi.oauth] callback read failed code={}",
                bangumiErrorCodeName(request.error().code));
    sendBrowserResponse(*socket, 400,
                        QByteArrayLiteral("Authorization callback rejected."));
    co_return ilias::Err(std::move(request.error()));
  }

  auto code = detail::parseBangumiCallbackRequest(
      *request, mExpectedCallbackPath, mExpectedState);
  if (!code) {
    AL_LOG_WARN("[bangumi.oauth] callback validation failed code={}",
                bangumiErrorCodeName(code.error().code));
    sendBrowserResponse(*socket, 400,
                        QByteArrayLiteral("Authorization callback rejected."));
  ...
  sendBrowserResponse(
      *socket, 200,
      QByteArrayLiteral(
          "Bangumi authorization received. You may close this page."));
  co_return *code;
}

auto BangumiAuth::readCallbackRequest(QTcpSocket &socket)
  ...
  while (!request.contains("\r\n\r\n")) {
    ...
    if (disconnected.has_value() && !ready.has_value()) {
      ...
      if (!request.contains("\r\n\r\n")) {
        co_return ilias::Err(
            bangumiError(BangumiErrorCode::InvalidCallback,
                         QStringLiteral("OAuth 回调连接在请求完成前关闭")));
      }
    }
  }
  co_return request;
...
auto BangumiAuth::exchangeCode(const QString &code, const QString &state)
    -> ilias::Task<BangumiResult<BangumiToken>> {
  ...
  const BangumiTokenExchangeRequest exchange{
      ...
      .clientSecret = qString(mSettings.client_secret),
      .code = code,
  ...
  mActiveReply.clear();
  AL_LOG_INFO("[bangumi.oauth] token exchange response status={} bytes={} "
  ...
  if (data.size() > kMaximumTokenResponseSize) {
    ...
    co_return ilias::Err(bangumiError(BangumiErrorCode::InvalidResponse,
                                      QStringLiteral("Token 响应过大")));
  }
  if (auto configuration =
          tokenExchangeConfigurationError(data, status, mSettings)) {
    AL_LOG_WARN("[bangumi.oauth] token exchange rejected application "
                "configuration");
  ...
  if (status < 200 || status >= 300) {
    ...
    co_return ilias::Err(
        bangumiError(BangumiErrorCode::NetworkError,
                     QStringLiteral("Token 交换返回 HTTP %1").arg(status)));
  }
  ...
  if (auto error = bangumi_protocol::decode(data, response)) {
    ...
    co_return ilias::Err(bangumiError(
        ...
        QStringLiteral("Token 响应不是有效 JSON：%1").arg(*error)));
  }
  if (response.accessToken.isEmpty() || response.refreshToken.isEmpty() ||
    ...
    co_return ilias::Err(
        bangumiError(BangumiErrorCode::InvalidResponse,
                     QStringLiteral("Token 响应缺少必需字段或字段类型不正确")));
  }
...
void BangumiAuth::sendBrowserResponse(QTcpSocket &socket, int status,
