...
auto invalidStoredCredential(QString message) -> BangumiResult<std::optional<BangumiToken>> {
    return ilias::Err(bangumiError(BangumiErrorCode::CredentialStoreError, std::move(message)));
}
 ...
 */
auto decodeStoredToken(const QByteArray &data) -> BangumiResult<std::optional<BangumiToken>> {
    BangumiToken token;
    if (auto error = bangumi_protocol::decode(data, token)) {
        auto message = QStringLiteral("保存的凭据不是有效的 JSON：%1").arg(*error);
        return invalidStoredCredential(message);
    }
    if (token.accessToken.isEmpty() || token.refreshToken.isEmpty() ||
        token.tokenType.isEmpty() || token.userId < 0 || token.expiresAt < 0) {
        return invalidStoredCredential(QStringLiteral("保存的凭据缺少必需字段或字段类型不正确"));
    }
...
auto encodeStoredToken(const BangumiToken &token) -> BangumiResult<QByteArray> {
    ...
    if (!data) {
        return ilias::Err(bangumiError(BangumiErrorCode::CredentialStoreError, QStringLiteral("无法把 Bangumi 凭据序列化为 RapidJSON")));
    }
...
public:
    ...
    auto load() -> ilias::Task<BangumiResult<std::optional<BangumiToken>>> override {
        AL_LOG_DEBUG("[bangumi.credentials] load backend=memory present={}",
                     mToken.has_value());
        co_return mToken;
    }

    auto save(const BangumiToken &token) -> ilias::Task<BangumiResult<void>> override {
        if (mToken) {
            clearBangumiToken(*mToken);
        }
        mToken = token;
...
public:
    ...
    auto load() -> ilias::Task<BangumiResult<std::optional<BangumiToken>>> override {
        ...
        return ilias::toTask(ilias::blocking([path = mPath]() {
            ...
            if (!file.open(QIODevice::ReadOnly)) {
                auto message = QStringLiteral("无法读取凭据文件 %1：%2").arg(path, file.errorString());
                return invalidStoredCredential(message);
            }
            if (file.size() <= 0 || file.size() > kMaximumCredentialFileSize) {
                auto message = QStringLiteral("凭据文件大小无效：%1 bytes").arg(file.size());
                return invalidStoredCredential(message);
            }
    ...
    auto save(const BangumiToken &token) -> ilias::Task<BangumiResult<void>> override {
        ...
        co_return co_await ilias::blocking([path = mPath, data = std::move(*serialized)]() -> BangumiResult<void> {
            AL_LOG_DEBUG("[bangumi.credentials] save started backend=file path={}", path.toStdString());
            const QFileInfo info(path);
            QDir directory = info.dir();
            if (!directory.exists() && !directory.mkpath(QStringLiteral("."))) {
                auto message = QStringLiteral("无法创建凭据目录：%1").arg(directory.path());
                return ilias::Err(bangumiError(BangumiErrorCode::CredentialStoreError, message));
            }
            ...
            if (!file.open(QIODevice::WriteOnly) || file.write(data) != data.size() ||
                !file.commit()) {
                auto message = QStringLiteral("无法保存凭据文件 %1：%2").arg(path, file.errorString());
                return ilias::Err(bangumiError(BangumiErrorCode::CredentialStoreError, message));
            }
            ...
            if (!QFile::setPermissions(path, QFileDevice::ReadOwner |
                ...
                QFile::remove(path);
                auto message = QStringLiteral("无法限制凭据文件权限，已删除该文件：%1").arg(path);
                return ilias::Err(bangumiError(BangumiErrorCode::CredentialStoreError, message));
            }
    ...
    auto clear() -> ilias::Task<BangumiResult<void>> override {
        return ilias::toTask(ilias::blocking([path = mPath]() -> BangumiResult<void> {
            if (!QFile::exists(path) || QFile::remove(path)) {
                AL_LOG_INFO("[bangumi.credentials] clear completed backend=file");
                return {};
            }
            auto message = QStringLiteral("无法删除凭据文件：%1").arg(path);
            return ilias::Err(bangumiError(BangumiErrorCode::CredentialStoreError, message));
        }));
    }
...
public:
    ...
    auto load() -> ilias::Task<BangumiResult<std::optional<BangumiToken>>> override {
        ...
        auto &data = **raw;
        auto result = decodeStoredToken(data);
        if (!data.isEmpty()) {
    ...
    auto clear() -> ilias::Task<BangumiResult<void>> override {
        auto cleared = co_await ilias::blocking([this]() { return mProvider->clear(); });
        if (cleared) {
...
auto detail::createSystemTokenStore(std::unique_ptr<SystemCredentialProvider> provider) -> std::unique_ptr<TokenStore> {
...
auto TokenStore::create(TokenStoreOptions options) -> BangumiResult<std::unique_ptr<TokenStore>> {
    if (options.kind != TokenStoreKind::File && !options.filePath.isEmpty()) {
        return ilias::Err(bangumiError(BangumiErrorCode::InvalidConfiguration, QStringLiteral("--token-file 只能与 file TokenStore 一起使用")));
    }
    ...
    switch (options.kind) {
        ...
        case TokenStoreKind::File:
            if (options.filePath.isEmpty()) {
                options.filePath = QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation) + QStringLiteral("/credentials.json");
            }
    ...
    return ilias::Err(bangumiError(BangumiErrorCode::InvalidConfiguration, QStringLiteral("无效的 TokenStore 类型")));
}

void clearBangumiToken(BangumiToken &token) noexcept {
    auto clear = [](QString &value) {
        if (!value.isEmpty()) {
            sodium_memzero(value.data(), static_cast<std::size_t>(value.size()) * sizeof(QChar));
            value.clear();
            value.squeeze();
        }
    };
