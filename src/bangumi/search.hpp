...
constexpr auto isValidBangumiSubjectSearchSort(BangumiSubjectSearchSort sort) noexcept -> bool {
    switch (sort) {
...
struct BangumiSearchSubjectTag {
    ...
    struct Neko {
        static constexpr auto value = Object(
            "name",  &BangumiSearchSubjectTag::name,
            "count", &BangumiSearchSubjectTag::count
        );
    };
    // clang-format on
};
...
struct BangumiSearchSubjectRating {
    ...
    struct Neko {
        static constexpr auto value = Object(
            "rank",  &BangumiSearchSubjectRating::rank,
            "total", &BangumiSearchSubjectRating::total,
            "score", &BangumiSearchSubjectRating::score
        );
    };
    // clang-format on
};
 ...
 * Neko 结构和字段类型校验；未知 numeric enum 和服务端数值按原样保留。
 */
...
auto buildBangumiSubjectSearchRequest(const BangumiSettings &settings, const BangumiSubjectSearchQuery &query,
                                      std::optional<QString> accessToken = std::nullopt) -> BangumiResult<BangumiHttpRequest>;
...
struct CustomParser<anime_land::BangumiSubjectSearchSort> {
    ...
    static auto write(W &writer, anime_land::BangumiSubjectSearchSort value, const Parent &parent, const Tags &tags) -> ParserResult {
        if (!anime_land::isValidBangumiSubjectSearchSort(value)) {
            return detail::parser_error(sa::ErrorCode::InvalidType, "Invalid Bangumi subject search sort");
        }
        const auto name = anime_land::bangumiSubjectSearchSortName(value);
        const QString encoded = QString::fromLatin1(name.data(), static_cast<qsizetype>(name.size()));
        return parser_write<W>(writer, encoded, parent, tags);
    }

    template <typename R, typename Tags>
    static auto read(typename R::InputValueType input, anime_land::BangumiSubjectSearchSort &value, const Tags &tags) -> ParserResult {
        QString raw;
