...
BangumiModule::BangumiModule(BangumiSettings settings, std::unique_ptr<TokenStore> tokenStore,
    ...
    QObject::connect(&mAuth, &BangumiAuth::phaseChanged, this, &BangumiModule::onAuthPhaseChanged);
...
auto BangumiModule::oauthApplicationGuide() const -> BangumiOAuthApplicationGuide {
    return buildBangumiOAuthApplicationGuide(mSettings, mOptions);
}

void BangumiModule::setOAuthApplication(const BangumiOAuthApplication &application) {
...
auto BangumiModule::restoreSession() -> ilias::Task<BangumiResult<BangumiUser>> {
    ...
    if (!isLogout()) {
        ...
        co_return ilias::Err(bangumiError(BangumiErrorCode::InvalidState, QStringLiteral("当前 Bangumi 状态不允许恢复会话")));
    }
    ...
    if (!*stored) {
        ...
        setState(BangumiLoginState::LoggedOut);
        co_return ilias::Err(bangumiError(BangumiErrorCode::NotLoggedIn, QStringLiteral("没有可恢复的 Bangumi 凭据")));
    }
    ...
    if (token.expiresAt <= QDateTime::currentSecsSinceEpoch()) {
        ...
        setState(BangumiLoginState::LoggedOut);
        co_return ilias::Err(bangumiError(BangumiErrorCode::TokenExpired, QStringLiteral("Bangumi Access Token 已过期，请重新登录")));
    }
    ...
    if (token.userId != 0 && token.userId != user->id) {
        ...
        clearBangumiToken(token);
        setState(BangumiLoginState::LoggedOut);
        co_return ilias::Err(bangumiError(BangumiErrorCode::InvalidResponse, QStringLiteral("本地 token 用户与 /v0/me 用户不一致")));
    }
...
auto BangumiModule::login() -> ilias::Task<BangumiResult<BangumiUser>> {
    ...
    if (!isLogout()) {
        AL_LOG_WARN("[bangumi.auth] login rejected state={}",
                    bangumiLoginStateName(mState));
        co_return ilias::Err(bangumiError(BangumiErrorCode::InvalidState, QStringLiteral("请先结束当前操作或退出已有 Bangumi 账号")));
    }
...
auto BangumiModule::searchSubjects(BangumiSubjectSearchQuery query)
    ...
    AL_LOG_INFO("[bangumi.search] fetch started limit={} offset={} authenticated={}", query.limit, query.offset, mToken.has_value());
    if (mNetworkConfigurationError) {
        co_return ilias::Err(*mNetworkConfigurationError);
    }
...
auto BangumiModule::getCurrentUserCollections(BangumiCollectionQuery query)
    ...
    if (mState != BangumiLoginState::LoggedIn || !mToken || !mUser) {
        ...
        co_return ilias::Err(bangumiError(BangumiErrorCode::NotLoggedIn, QStringLiteral("请先登录 Bangumi 再获取用户收藏")));
    }
    ...
    if (feature == nullptr) {
        ...
        co_return ilias::Err(bangumiError(BangumiErrorCode::InvalidState, QStringLiteral("获取用户收藏功能未在 BangumiModuleOptions 中注册")));
    }

    auto result = co_await mClient.getUserCollections(*mToken, mUser->username, query, *feature);
    if (!result) {
...
void BangumiModule::setState(BangumiLoginState state) {
    ...
    mState = state;
    AL_LOG_INFO("[bangumi.module] state {} -> {}", bangumiLoginStateName(previous), bangumiLoginStateName(state));
...
auto BangumiModule::verifyAndCommit(BangumiToken token) -> ilias::Task<BangumiResult<BangumiUser>> {
    ...
    if (token.userId != 0 && token.userId != user->id) {
        ...
        clearBangumiToken(token);
        setState(BangumiLoginState::LoggedOut);
        co_return ilias::Err(bangumiError(BangumiErrorCode::InvalidResponse, QStringLiteral("Token 响应用户与 /v0/me 用户不一致")));
    }
    ...
    if (!saved) {
        ...
        AL_LOG_ERROR("[bangumi.auth] token persistence failed code={}; session remains active", bangumiErrorCodeName(saved.error().code));
    }
