...
auto configureBangumiNetworkProxy(QNetworkAccessManager &network, const BangumiSettings &settings) -> BangumiResult<bool> {
    ...
    if (url.isEmpty()) {
        if (!settings.proxy_username.isEmpty() || !settings.proxy_password.empty()) {
            return invalidProxy(QStringLiteral("已配置 Bangumi 代理凭据，但 proxy_url 为空"));
        }
        return false;
    }
    ...
    if (!url.userName().isEmpty() || !url.password().isEmpty()) {
        return invalidProxy(QStringLiteral("请使用 proxy_username 和 proxy_password 配置 Bangumi 代理凭据"));
    }
    if ((!url.path().isEmpty() && url.path() != QStringLiteral("/")) ||
        url.hasQuery() || url.hasFragment()) {
        return invalidProxy(QStringLiteral("Bangumi proxy_url 不能包含路径、查询参数或片段"));
    }
    if (settings.proxy_username.isEmpty() && !settings.proxy_password.empty()) {
        return invalidProxy(QStringLiteral("Bangumi proxy_password 需要同时配置 proxy_username"));
    }
    ...
    if (scheme == QStringLiteral("http")) {
        ...
        return invalidProxy(QStringLiteral("Bangumi proxy_url 仅支持 http:// 或 socks5://"));
    }
    ...
    const QByteArrayView encodedPassword(settings.proxy_password.data(), static_cast<qsizetype>(settings.proxy_password.size()));
    ...
    if (passwordDecoder.hasError()) {
        return invalidProxy(QStringLiteral("Bangumi proxy_password 必须是有效 UTF-8"));
    }

    network.setProxy(QNetworkProxy(type, url.host(), static_cast<std::uint16_t>(port), settings.proxy_username, password));
    return true;
}
