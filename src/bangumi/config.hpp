...
constexpr auto operator|(BangumiCapability left,
                         BangumiCapability right) noexcept
    -> BangumiCapability {
...
constexpr auto hasBangumiCapability(BangumiCapability capabilities,
                                    BangumiCapability required) noexcept
    -> bool {
...
template <typename T> using BangumiResult = ilias::Result<T, BangumiError>;
...
public:
   ...
   */
  virtual auto load()
      -> ilias::Task<BangumiResult<std::optional<BangumiToken>>> = 0;
   ...
   */
  virtual auto save(const BangumiToken &token)
      -> ilias::Task<BangumiResult<void>> = 0;
   ...
   */
  static auto create(TokenStoreOptions options)
